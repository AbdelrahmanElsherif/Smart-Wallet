...
MyThread::MyThread(std::unordered_map<int,Client>&mp1,std::unordered_map<QString,Client>&mp2,int ID, QObject *parent) :
    QThread(parent),
    client(mp1),
    credentianls(mp2)
{
...
void MyThread::readyRead()
{
     ...
     if(header=="client"){
        ...
        if(credentianls.count(uname)!=0){
            socket->write("username already taken");
            return;
        }
        credentianls[uname]= newclient;
        ...
        client[socketDescriptor]=newclient;
         ...
         if(credentianls.count(uname)==0){qDebug()<<"username not found";socket->write("please check username and password"); return;}
         else{
             ...
             if(client[socketDescriptor].getUserName()==uname&&client[socketDescriptor].getPassword()==password){
                 ...
                 return;}
                     ...
                     Client oldclient=client[socketDescriptor];
                     client[index]=oldclient;
                     client[socketDescriptor]=credentianls[uname];
                     socket->write("login successful");
                 }
             }
         ...
         credentianls[client[socketDescriptor].getUserName()].account.SetBalance(newbalance);
         Transaction t;
         t.TransType=deposit;
         t.TransAmount=deposited;
         client[socketDescriptor].TransStack.push(t);
         credentianls[client[socketDescriptor].getUserName()].TransStack.push(t);
         ...
         if(!client[socketDescriptor].TransStack.empty()){
             ...
             if(last.TransType==deposit){
                 ...
                 if(withdrawn<currentBalance){
                    ...
                    credentianls[client[socketDescriptor].getUserName()].TransStack.pop();
                 }else{
